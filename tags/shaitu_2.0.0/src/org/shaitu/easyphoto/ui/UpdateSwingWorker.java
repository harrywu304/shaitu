...
public class UpdateSwingWorker extends SwingWorker {
	 ...
	 */
	public static ResourceBundle messageMapping;
	/**
	...
	public UpdateSwingWorker(EasyPhoto main) {
		...
		messageMapping = main.messageMapping;
	}
	...
	protected void done() {
		...
		if ("checkUpdate".equals(actionType)) {
			...
			if (!action.isCheckDone()) {
				if(!isCycleCheck){
					dialogInfo = messageMapping
							.getString("dialog.update.checkfail");
			...
			if (!action.isUpdateValid()) {
				if(!isCycleCheck){
					dialogInfo = messageMapping.getString("dialog.update.noupdate");
					JOptionPane.showMessageDialog(main, dialogInfo);
				}
				return;
			}
			...
			if (action.isAutoUpdateValid()) {
				dialogInfo = MessageFormat.format(messageMapping
						.getString("dialog.update.autoupdate"), action
						.getLatestVersion());
				int n = JOptionPane.showConfirmDialog(main, dialogInfo,
						messageMapping.getString("dialog.update.title"),
			...
			dialogInfo = MessageFormat.format(messageMapping
					.getString("dialog.update.needreinstall"), action
					.getLatestVersion());
			int n = JOptionPane.showConfirmDialog(main, dialogInfo,
					messageMapping.getString("dialog.update.title"),
					JOptionPane.OK_CANCEL_OPTION,
			...
			int n = JOptionPane.showConfirmDialog(main, messageMapping
					.getString("dialog.update.restart"), messageMapping
